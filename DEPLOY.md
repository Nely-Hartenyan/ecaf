...
2. **Заполните базу данных администраторами** (ОБЯЗАТЕЛЬНО!):

   ```bash
   # Вариант 1: Запустить seed скрипт (создаст 3 админов)
   npx prisma db seed
   
   # Вариант 2: Создать одного админа
   npm run db:create-admin
   ```

   Это создаст администраторов:
   - `admin1@college.am` / `Admin12345!`
   - `admin2@college.am` / `Admin12345!`
   - `admin3@college.am` / `Admin12345!`

   ⚠️ **Важно:** После первого входа обязательно измените пароли!
